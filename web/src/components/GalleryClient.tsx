...
import { MoveRight, X, Heart, Camera, PartyPopper, Baby, CalendarDays, Play } from "lucide-react";
...
export default function GalleryClient({
    ...
    return (
        <section ref={containerRef} id="portfolio" className={`relative bg-black overflow-hidden ${hideTabs ? 'pt-0' : 'pt-20 md:pt-32'}`}>
                    ...
                    <>
                        <div className="mb-12 md:mb-24 flex flex-col md:flex-row md:items-end justify-between gap-6 md:gap-10">
                            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 1 }} viewport={{ once: true }}>
                                <span className="text-[#D4AF37] text-[9px] md:text-[10px] uppercase tracking-[0.6em] font-bold mb-4 block">Archive</span>
                                <h2 className="text-4xl md:text-7xl font-sans font-black uppercase text-white leading-[0.9] tracking-tighter">
                                    VISUAL <br /> <span className="font-serif italic font-light lowercase text-white/30">anthology.</span>
                                </h2>
                            </motion.div>
                        ...
                        {/* Category Scroller - Hidden if hideTabs is true */}
                        ...
                        >
                            <AnimatePresence mode="popLayout">
                                {filteredImages.filter((_, i) => i % 3 === colIndex).map((img) => (
                                    <motion.div key={img._id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="group relative">
                                        <div
                                            onClick={() => setSelectedImage({ src: img.src, alt: img.title, video: img.videoUrl })}
                                        ...
                                        >
                                            ...
                                            <div className="absolute bottom-0 left-0 w-full p-6 md:p-8 md:translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                                                ...
                                                <h3 className="text-white text-xl md:text-2xl font-serif italic">{img.title}</h3>
            ...
            {/* Footer CTA - Hidden if hideTabs is true */}
            {!hideTabs && (
